(() => {
  ...
  // Experiment 6 uses one of each color; Experiment 7 uses two of each color.
  const replacementBalls = ["Red", "Green", "Blue"];
  ...
  const config = {
    ...
    ballRep: {
      ...
      instruction: "The cup contains 1 Red, 1 Green, and 1 Blue ball. Draw one ball, replace it, then draw a second ball.",
  ...
  function showReadyVisual() {
    ...
    } else if (mode === "ballRep") {
      visual.innerHTML = `<div class="ball-process">${cupHTML(replacementBalls)}</div>`;
  ...
  async function animateTwoBallTrial(withReplacement) {
    if (withReplacement) {
      // First draw is from the 3 balls: one Red, one Green, and one Blue.
      const first = replacementBalls[rand(replacementBalls.length)];

      visual.innerHTML = `
        <div class="ball-process">
          ${cupHTML(replacementBalls, "mixing")}
          <div class="draw-zone">
      ...
      const afterFirstDraw = removeOne(replacementBalls, first);
      ...
      const available = [...replacementBalls];
      ...
      detail.textContent = `${first} is replaced. The cup has 1 Red, 1 Green, and 1 Blue ball again.`;
      await sleep(900);
