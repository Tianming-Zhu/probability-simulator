(() => {
  ...
  const config = {
    ...
    ballRep: {
      ...
      instruction: "Draw one ball, replace it, then draw a second ball.",
      outcomes: ["Same color", "Different colors"]
    },
    ballNoRep: {
      title: "Drawing Two Balls without Replacement",
      instruction: "Draw one ball and keep it out before drawing the second ball.",
      outcomes: ["Same color", "Different colors"]
    }
  };
  ...
  function cupHTML(ballColors = colors, extraClass = "") {
    return `
      <div class="cup-zone ${extraClass}">
        <div class="cup-label">Cup</div>
        <div class="cup">
          <div class="cup-balls">
            ${ballColors.map(c => ballHTML(c)).join("")}
          </div>
        </div>
      </div>`;
  }
  ...
  function showReadyVisual() {
    ...
    if (mode === "coin1") {
      ...
      visual.innerHTML = `<div class="ball-process">${cupHTML(colors)}</div>`;
    } else {
  ...
  async function animateTwoBallTrial(withReplacement) {
    const first = colors[rand(3)];

    visual.innerHTML = `
      <div class="ball-process">
        ${cupHTML(colors, "mixing")}
        <div class="draw-zone">
    ...
    visual.innerHTML = `
      <div class="ball-process">
        ${cupHTML(colors.filter(c => c !== first))}
        <div class="draw-zone">
          <div class="draw-label">First draw</div>
          ${ballHTML(first)}
        </div>
    ...
    if (withReplacement) {
      ...
      detail.textContent = `${first} is replaced. The cup has Red, Green, and Blue again.`;
      available = colors;
    } else {
      available = colors.filter(c => c !== first);
      visual.innerHTML = `
        <div class="ball-process">
          ${cupHTML(available)}
          <div class="draw-zone">
            <div class="draw-label">First draw</div>
            ${ballHTML(first)}
            <div class="status-badge keep-out">Kept out</div>
          </div>
        </div>`;
      detail.textContent = `${first} is not replaced. Only ${available.join(" and ")} remain in the cup.`;
    }
