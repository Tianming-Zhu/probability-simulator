(() => {
  ...
  const config = {
    ...
    ballNoRep: {
      ...
      instruction: "The cup contains 2 Red, 2 Green, and 2 Blue balls. Draw one ball and keep it out before drawing the second ball.",
      outcomes: ["Same color", "Different colors"]
    }
  };
  ...
  async function animateTwoBallTrial(withReplacement) {
    ...
    // One physical ball has been removed for the moment.
    ...
    if (withReplacement) {
      ...
      // Only one copy of the first ball is removed.
      // Because there were two of each color, one matching-color ball still remains.
      available = afterFirstDraw;
      visual.innerHTML = `
        <div class="ball-process">
          ${cupHTML(available)}
          <div class="draw-zone">
      ...
      const remainingSameColor = available.filter(c => c === first).length;
      detail.textContent = `${first} is not replaced. 5 balls remain, including ${remainingSameColor} ${first} ball.`;
    }
