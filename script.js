(() => {
  ...
  const config = {
    ...
    ballNoRep: {
      ...
      instruction: "Draw one ball and do not replace it before drawing the second ball.",
      outcomes: ["Same color", "Different colors"]
    }
  };
  ...
  function makeResult() {
    ...
    if (mode === "ballRep") {
      const a = colors[rand(3)];
      const b = colors[rand(3)];
      outcome = a === b ? "Same color" : "Different colors";
      detailText = `First: ${a} · Second: ${b}`;
      html = `<div class="row">
        <div class="ball ${a.toLowerCase()}">${a}</div>
        <div class="ball ${b.toLowerCase()}">${b}</div>
      </div>`;
    }

    if (mode === "ballNoRep") {
      const a = colors[rand(3)];
      const remaining = colors.filter(c => c !== a);
      const b = remaining[rand(2)];
      outcome = "Different colors";
      detailText = `First: ${a} · Second: ${b}`;
      html = `<div class="row">
        <div class="ball ${a.toLowerCase()}">${a}</div>
        <div class="ball ${b.toLowerCase()}">${b}</div>
